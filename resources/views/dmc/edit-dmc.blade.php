...
<?php 
// foreach ($dmc->DMCFleets as $key => $value) {
// 	print_b($value->FleetDescriptions);
// }
// print_b($dmc->DMCFleets); 
		...
		<div class="row">
			<div class="col-12">
				<div class="panel">
					...
					<form action="{{ route('post_edit',['id'=>$dmc->id ?? '']) }}" method="post" enctype="multipart/form-data">
					@csrf
					<input type="hidden" value="{{ $dmc->latitude ?? old('latitude') }}" name="latitude" id="latitude" />
                    <input type="hidden" value="{{ $dmc->longitude ?? old('longitude') }}" name="longitude" id="longitude" />
					...
					<div class="row">
						<div class="col-sm-12 col-md-10 col-lg-7">
							...
							<?php //print_b($dmc->DMCDescriptions); exit; ?>

							<?php if($languages->count() && !empty($dmc->DMCDescriptions) ): ?>
                                <?php for ($i=0; $i <3 ; $i++): ?>
                                    ...
                                    <div class="row mtpx-30">
                                        <div class="col-sm-12">
                                            <div class="field ">
                                                ...
                                                <input type="text" name="dmc_name-{{ $languages[$i]->id }}" placeholder="DMC Name " value="{{ $dmc->DMCDescriptions[$i]['name'] ?? old('dmc_name-'.$languages[$i]->id) }}" />
                                            </div>
                                        </div>
                            		</div>
                                <?php endfor; ?>
							...
							@if( !empty($dmc->location) )
							<div class="row">
								<div class="col-md-12">
									<div class="field">
										<label>Location</label>
										<input type="text" name="dmc_location" id="dmc_location" value="{{ $dmc->location ?? '' }}">
									</div>
								</div>
							</div>
							@endif
							<div class="row">
								...
								<div class="col-md-4">
									<div class="field">
										<label>City</label>
										<input type="text" name="city" id="locality" value="{{ $dmc->city ?? '' }}">
								...
								<div class="col-md-4">
									<div class="field">
										<label>State</label>
										<input type="text" name="state" id="administrative_area_level_1" value="{{ $dmc->state ?? '' }}">
									</div>
								</div>
								@endif
								@if( !empty($dmc->zip) )
								<div class="col-md-4">
									<div class="field">
										<label>Zip</label>
										<input type="text" name="zip" id="postal_code" value="{{ $dmc->zip ?? '' }}">
									</div>
								</div>
								@endif
							</div>
							...
							<div class="to-clone">
								...
								@if( !empty($dmc->DMCSection) )
									@foreach( $dmc->DMCSection as $section )
								<div class="row">
									...
									<div class="col-md-6">
										<div class="field wico altr-b">
											<div class="addFileStyle drag">
											    ...
											    <figure>
													<img src="{{ asset('uploads/dmc/'.$dmc->id.'/'.$section->image) }}" />
												</figure>
											</div>
										</div>
									...
									<div class="col-md-6">
			                       	...
			                       	<?php if($languages->count()): ?>
		                                ...
		                                <?php for ($i=0; $i <$languages->count() ; $i++): ?>
		                                    <?php $iso = $languages[$i]->{'iso_639-1'}; ?>
		                                    <div class="row mtpx-30">
		                                        <div class="col-sm-12">
		                                            <div class="field lang-{{ $iso }}">
		                                                <label>Heading</label>
		                                                <input type="text" name="heading-{{ $languages[$i]->id }}[{{$section_to_count}}]" placeholder="Heading {{ $languages[$i]->name }}" value="<?php echo $section->DMCSectionDescriptions[$i]->title ?? old('heading-'.$languages[$i]->id) ?>" />
		                                            </div>
		                                        </div>
		                                    </div>
		                                    ...
		                                    <div class="row mtpx-30">
			                                        <div class="col-sm-12">
			                                            <div class="field lang-{{ $iso }}">
			                                                ...
			                                                <textarea rows="2" cols="2" name="paragraph-{{ $languages[$i]->id }}[{{$section_to_count}}]">{{ $section->DMCSectionDescriptions[$i]->description ?? '' }}</textarea>
			                                            </div>
			                                        </div>
			                                 </div>
		                                <?php endfor; ?>
									...
									<div class="col-md-6">
										<div class="field wico altr-b">
											<div class="addFileStyle drag-f">
											    <input type="file" accept="image/*" name="fleet_image[]"  class="fleet_image" />
											</div>
										</div>
									</div>
									<div class="col-md-6">
							...
							<input type="hidden" value="" name="section_bottom[]">
								<div class="row">
									<div class="col-md-6">
										<div class="field wico altr-b">
											<div class="addFileStyle drag">
											    <input type="file" name="section_image_bottom[]" />
											</div>
										</div>
									</div>
									<div class="col-md-6">
...
@section('footer_script')
            ...
            <script type="text/javascript">
				...
				section_bottom+='<div class="addFileStyle drag"><div class="upldwrap"><input type="file" name="section_image_bottom[]" style="opacity: 0;"><input type="text"> <input type="button" value="Browse" class="grey_btn"></div></div>';
				section_bottom+='</div>';
			...
			<?php if($languages->count()): ?>
				...
				fleet+='<div class="addFileStyle drag"><div class="upldwrap"><input type="file" name="fleet_image[]" style="opacity: 0;"><input type="text"> <input type="button" value="Browse" class="grey_btn"></div></div>';
				fleet+='</div>';
			...
			<?php if($languages->count()): ?>
				...
				section_1+='<div class="addFileStyle drag"><div class="upldwrap"><input type="file" name="section_image[]" style="opacity: 0;"><input type="text"> <input type="button" value="Browse" class="grey_btn"></div></div>';
				section_1+='</div>';
			...
			<?php if($languages->count()): ?>
                ...
                $(document).on('change',".section_image",function(){
                    readURL(this,this.n);
                });

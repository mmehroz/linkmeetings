                ...
                @endif
                    ...
                    <div class="bx">
                        <div class="head">
                            ...
                            @endif
                            @if($key == 1)
                                <h2 class="green">{{ $dmc->title ?? '' }}</h2>
                            @endif
                            @if($key == 2)
                                <h2 class="orange">{{ $dmc->title ?? '' }}</h2>
                            @endif
                            <h3>{{ $dmc->short_title ?? '' }}</h3>
                        </div>
                        <div class="price">
                            <h5>${{ $dmc->price ?? '' }} USD<span>/year</span></h5>
                        </div>
                        <div class="dt">
                            {!! $dmc->details ?? '' !!}
                            <div class="clearfix">
                                <a href="{{ route('get_package',['id'=>$dmc->id ??'','type'=>'dmc']) }}" class="primary-btn dblue">Get Started</a>
                            </div>
                        </div>
                    </div>
                </div>
